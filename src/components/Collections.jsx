...
function Collections({ collectionAddress, setcollectionAddress }) {
  const { CollectionNFTs, getCollections } = useCollections(collectionAddress);
  ...
  return (
    <div>
      <div>
      { collectionAddress !== "explore" &&  <Button value="Go Back" onClick={() => setcollectionAddress("explore")}>
          Go Back
        </Button> } 
      </div>
      <br />
      <div style={styles.NFTs}>
        ...
        {NFTMCollections &&
          collectionAddress === "explore" &&
          NFTMCollections.map((nft, index) => (
            <Card
              hoverable
              actions={[
                <Tooltip title="View This Collection">
                  <RightCircleOutlined
                    onClick={() => setcollectionAddress(nft.addrs)}
                  />
                </Tooltip>,
              ]}
              style={{  width: 300, border: "2px solid #1890ff",  margin: "20px",
              borderRadius: "20px",
              overflow: "hidden"}}
              cover={
                <Image
      ...
      {getMarketItem(nftToBuy) ? (
        ...
        >
          ...
          />
          <Alert style={{ fontWeight: "400"}} message="This NFT is currently not for sale" type="info" />
        </Modal>
      )}
    </div>
  );
